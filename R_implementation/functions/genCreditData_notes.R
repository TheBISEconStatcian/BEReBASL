...
genCreditData <- function(n                = 1000, 
  ...
  for (combo in combos) { #combos = '1', as k_bin = 0
    
    # distribution parameters
    combo_vals      <- as.numeric(unlist(strsplit(combo, split = '')))
    combo_idx       <- apply(dummy, 1, function(x) all(x == combo_vals)) # dummy == data.frame(D1 = rep(1, n))
    combo_count     <- sum(combo_idx)
    if (sum(combo_vals) > 0 & k_bin != 0) {
      combo_bad_ratio <- mean(bin_bad_ratio * combo_vals)
    }else{
      combo_bad_ratio <- bad_ratio # This happens deterministically in each iteration
    }
    
    # compute sample size
    combo_n1 <- round(combo_count * combo_bad_ratio) # n * 0.7
    combo_n2 <- round(combo_count * (1 - combo_bad_ratio)) # n * 0.3
    ...
    if (is.null(replicate)) {
      mu1 <- rep(sum(combo_vals * bin_mean_con_dif), k_con) # This should always be c(0,0), bin_mean_con_dif = numeric(0)
      mu2 <- mu1 + con_mean_bad_dif + sum(combo_vals * bin_mean_bad_dif) # This is then exactly con_mean_bad_dif <- c(1,2) 
      # bin_mean_bad_dif = numeric(0)
      if (mixture == T) {
