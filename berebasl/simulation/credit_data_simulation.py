...
class CreditDataSample(Dataset):
    ...
    def filter_unlabeled(
        ...
        """
        Filter unlabeled samples while preserving normalized dimensions.

        This method removes unlabeled samples according to ``keep_mask`` while
        maintaining the original batch structure. If all batches keep the same
        number of samples, a fast path is used that directly applies the mask.
        Otherwise, samples are compacted per batch, padded to ensure consistent
        shapes, and finally trimmed to the maximum number of kept samples.

        Parameters
        ----------
        keep_mask : torch.Tensor
            Boolean mask of shape ``(*super_batch_shape, N_unlabeled)`` indicating
            which unlabeled samples to keep. ``True`` means the sample is kept.
        inplace : bool, optional
            If ``True`` (default), modify this instance in place.
            If ``False``, return a cloned and filtered instance.

        Returns
        -------
        CreditDataSample
            The filtered instance (``self`` if ``inplace=True``; otherwise a clone).
        """
        if keep_mask.shape != self._unlabeled_ids.shape:
            raise AssertionError(
                f"keep_mask does not have the expected shape. "
                f"Expected: {self._unlabeled_ids.shape}, got: {keep_mask.shape}"
            )
        ...
        if not inplace:
            new = self.clone()
            return new.filter_unlabeled(keep_mask, inplace=True)
        ...
        # Compute per-batch kept counts
        counts = keep_mask.reshape(B, N).sum(dim=-1)
        ...
        if no_padding_needed:
            ...
            self._unlabeled_ids = (
                self._unlabeled_ids[keep_mask]
                .reshape(*batch_shape, new_N)
            )
            return self

        # ------------------------------------------------------------
        # SLOW PATH: batches keep different numbers of samples
        # ------------------------------------------------------------
        feats = self.features_unlabeled.reshape(B, N, F)
        ids = self._unlabeled_ids.reshape(B, N)
        mask = keep_mask.reshape(B, N)

        # Sort so kept entries (True) move to the end
        _, idx = mask.sort(dim=-1)
        rev_idx = idx.flip(-1)

        feats_sorted = feats.gather(1, rev_idx.unsqueeze(-1).expand(B, N, F))
        ids_sorted = ids.gather(1, rev_idx)

        arange = torch.arange(N, device=feats.device).unsqueeze(0)
        valid = arange < counts.unsqueeze(-1)

        feats_padded = feats_sorted.masked_fill(~valid.unsqueeze(-1), torch.nan)
        ids_padded = ids_sorted.masked_fill(~valid, self._nan_val_ids_rejects)

        # Restore batch shape
        feats_padded = feats_padded.reshape(*batch_shape, N, F)
        ids_padded = ids_padded.reshape(*batch_shape, N)

        # Trim to minimal consistent size
        new_N = int(counts.max().item())
        self.features_unlabeled = feats_padded[..., :new_N, :]
        self._unlabeled_ids = ids_padded[..., :new_N]
...
class CreditData(Dataset):
    ...
    def round_selection_mask(self, from_round_idx: Optional[int] = None, up_to_round_idx: Optional[int] = None):
        ...
        if from_round_idx > up_to_round_idx:
            raise ValueError("from_round_idx needs to be smaller than up_to_round_idx")
        if from_round_idx < 0:
            raise AssertionError("from_round_idx needs to be between 0 and self.last_gen_round")
